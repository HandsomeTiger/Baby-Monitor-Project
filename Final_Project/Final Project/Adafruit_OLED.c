...
void writeCommand(unsigned char c) {
       ...
       unsigned char C_Byte[1] = {c};
       unsigned char tmp[1];

       // Enable chip select
       SPICSEnable(GSPI_BASE);

       // Set data/command(DC) LOW
       GPIOPinWrite(GPIOA1_BASE, 0x2, 0x0);

       // Set chip select (OC) LOW
       GPIOPinWrite(GPIOA0_BASE, 0x40, 0x00);

       // Perform SPI transfer to send SPT
       SPITransfer(GSPI_BASE, C_Byte, tmp, 1, 0);
...
*/
    ...
    // Set RS pin HIGH to indicate a data transmission.

    // Set CS pin LOW to start communication with the OLED display.

    // Use SPI communication to send the data byte 'c'.
...
void halfScreen(unsigned int fillcolor) {
  fillRect(0, 0, SSD1351WIDTH, 64, fillcolor);
}
